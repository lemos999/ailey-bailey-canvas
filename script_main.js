...
--- Ailey & Bailey Canvas ---
File: script_main.js
Version: 13.1 (Event Delegation Patch)
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function attachEventListeners() {
        ...
        // --- Notes App Listeners (Re-architected with Event Delegation) ---
        if (noteListView) {
            noteListView.addEventListener('click', e => {
                // [PATCHED] Event Delegation for the dynamically created dropdown menu
                const dropdownAction = e.target.closest('.dropdown-item')?.dataset.action;
                if(dropdownAction) {
                    if (dropdownAction === 'export-all') {
                        exportAllData(); // Correctly calls the backup function
                    } else if (dropdownAction === 'import-all') {
                        handleRestoreClick(); // Correctly calls the restore function
                    } else if (dropdownAction === 'system-reset') {
                        handleSystemReset(); // Correctly calls the reset function
                    }
            ...
            // Delegation for context menu (future-proof)
            noteListView.addEventListener('contextmenu', e => {
                const projectHeader = e.target.closest('.note-project-header');
                if(projectHeader) {
                    e.preventDefault();
                    // Future: showNoteProjectContextMenu(projectHeader.closest('.note-project-container').dataset.projectId, e.clientX, e.clientY);
                }
            });
        }
        
        // This container is now inside noteListView, so no separate listener is needed.
        // Drag and drop listeners would also benefit from being on noteListView if they target dynamic items.
        ...
        // --- Modal & API Settings Listeners (for brevity, assume they are correct) ---
