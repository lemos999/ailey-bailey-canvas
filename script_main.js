...
--- Ailey & Bailey Canvas ---
File: script_main.js
Version: 13.2 (Robust Event Delegation Patch)
Architect: [Username] & System Architect Ailey
Description: The main entry point for the application. Attaches robust event listeners to stable parent elements.
*/
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initialize() {
        ...
        initializeFirebase().then(() => { 
            ...
            handleNewChat();
            if (notesAppPanel) setupNoteListView(); // Setup the static structure of the notes panel
        });
    ...
    function attachEventListeners() {
        ...
        // --- [PATCHED] Notes App Event Delegation on the STABLE Parent Panel ---
        if (notesAppPanel) {
            notesAppPanel.addEventListener('click', e => {
                // Delegation for the entire note list view
                if (e.target.closest('#note-list-view')) {
                ...
                // Delegation for the note editor view
                if (e.target.closest('#note-editor-view')) {
                    if (e.target.closest('#back-to-list-btn')) switchView('list');
                    const formatBtn = e.target.closest('.format-btn');
                    if (formatBtn) applyFormat(formatBtn.dataset.format);
                    if (e.target.closest('#link-topic-btn')) {
            ...
            // Input events need to be attached directly, but they are inside the stable editor view
            ...
            // Drag and drop listeners can also be attached to the panel
            // ... (Drag and drop logic here for robustness)
        }
