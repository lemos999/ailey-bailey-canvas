    ...
    function attachEventListeners() {
        ...
        if (notesAppToggleBtn) {
            notesAppToggleBtn.addEventListener('click', () => { 
                ...
                if (notesAppPanel.style.display === 'flex') { // If the panel is now open
                    ensureNotePanelHeader();
                    // [NEW ARCHITECTURE] 1. First, reset the view state completely to prevent conflicts.
                    switchView('list'); 
                    // [NEW ARCHITECTURE] 2. Then, render the list content into the now-clean view.
                    renderNoteList();
                }
            });
        }
