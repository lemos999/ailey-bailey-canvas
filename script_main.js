        ...
        if (notesAppToggleBtn) {
            notesAppToggleBtn.addEventListener('click', () => { 
                ...
                if (isOpening) {
                    ...
                    noteListView.classList.add('is-initializing'); // 1. Activate ghost click protection
                    switchView('list'); // 2. Reset view to a clean state
                    renderNoteList();   // 3. Render the new UI
                    
                    setTimeout(() => { // 4. Deactivate protection after event loop
                        noteListView.classList.remove('is-initializing');
                    }, 100);
                }
            });
        }
        
        // --- Notes App Event Delegation (Rewritten for new UI) ---
        if (noteListView) {
            ...
            noteListView.addEventListener('click', e => {
                ...
                const viewToggle = target.closest('.view-toggle-btn');
                if (viewToggle) {
                    currentNoteViewMode = viewToggle.dataset.view;
                    renderNoteList();
                    return;
                }
                
                // Action Bar: Main Buttons
                ...
                // Note Cards: Quick Actions
                const pinBtn = target.closest('.pin-btn');
                if (pinBtn) {
                    togglePin(pinBtn.closest('.note-card').dataset.id);
                    return;
                }
                const deleteBtn = target.closest('.delete-btn');
                if (deleteBtn) {
                    deleteNote(deleteBtn.closest('.note-card').dataset.id);
                    return;
                }
                
                // Note Cards: Open Editor
                const noteCardContent = target.closest('.note-card-content, .note-card-footer');
                if (noteCardContent) {
                    openNoteEditor(noteCardContent.closest('.note-card').dataset.id);
                    return;
                }

                // Projects: Toggle Expansion
