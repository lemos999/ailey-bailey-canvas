...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initialize() {
        // ... (Initialization code is largely unchanged)
    ...
    function attachEventListeners() {
        ...
        // ... (Other global listeners remain the same)
        ...
        // [PATCH] Step 3: Robust Event Listener for the ENTIRE Notes Panel
        if (notesAppPanel) {
            notesAppPanel.addEventListener('click', e => {
                const button = e.target.closest('button');
                if (!button) return; // If the click was not on a button, do nothing

                // Actions from the Action Bar or Dropdown
                const mainAction = button.dataset.action;
                if (mainAction) {
                    switch (mainAction) {
                        ...
                        case 'import-all': handleRestoreClick(); break;
                        case 'system-reset': handleSystemReset(); break;
                    }
                ...
                // Actions within a note item (Pin/Delete)
                ...
                if (noteItem) {
                    ...
                    if (button.classList.contains('pin-btn')) {
                        ...
                        openNoteEditor(noteId); // Click on item itself
                    }
                    return; // Action handled
                }
            });
            ...
            notesAppPanel.addEventListener('click', e => {
                // This is a separate handler to avoid complex logic in one.
                // It handles clicks that are NOT on buttons.
                if (e.target.closest('button')) return;

                const noteItem = e.target.closest('.note-item');
                if (noteItem) {
                    openNoteEditor(noteItem.dataset.id);
                    return;
                }
