...
export function openApiSettingsModal() {
    ...
    if (State.userApiSettings.apiKey) {
         modalElements.apiKeyStatus.textContent = +'✅ [] 키가 활성화되어 있습니다.'+;
...
export async function handleVerifyApiKey() {
    ...
    modalElements.apiKeyStatus.textContent = +'[] 키 검증 및 모델 목록 로딩 중...'+; 
    ...
    try {
        ...
        modalElements.apiKeyStatus.textContent = +'✅ [] 키 검증 완료! 모델을 선택하고 저장하세요.'+; 
        modalElements.apiKeyStatus.className = 'status-success'; 
        modalElements.apiModelSelect.disabled = false;
    } catch (error) {
        console.error("API Key Verification Error:", error);
        modalElements.apiKeyStatus.textContent = +'❌ [] 키 검증 실패: '+; 
...
export function renderTokenUsage() {
    ...
    modalElements.tokenUsageDisplay.innerHTML = +'<span>입력: </span> | <span>출력: </span> | <strong>총합: </strong>'+;
}
