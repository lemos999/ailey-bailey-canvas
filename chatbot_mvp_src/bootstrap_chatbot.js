...
(async () => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Config not found at ${BASE_URL}config_chatbot.json`);
    ...
    console.log("Chatbot MVP Initialized Successfully.");
  } catch (error) {
    console.error("Chatbot MVP failed to load:", error);
    document.body.innerHTML = `<div style="color:red;padding:20px;"><h1>Error</h1><p>Failed to load chatbot application.</p><pre>${error.stack}</pre></div>`;
  } finally {
