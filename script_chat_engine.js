...
Version: 12.1 (KaTeX Integration)
Architect: [Username] & System Architect Ailey
Description: The core engine for chat functionality. Handles message sending, API requests, response parsing, and rendering of dynamic elements like the reasoning block and KaTeX math formulas.
*/
        ...
        if (msg.role === 'user') {
            ...
            if (match) {
                ...
                if (finalAnswer) {
                    ...
                    finalAnswerDiv.textContent = finalAnswer; // Set as text first
                    renderMathInElement(finalAnswerDiv); // Then process for math
                    aiContainer.appendChild(finalAnswerDiv);
                }
            } else { // Simple AI message without reasoning
                const d = document.createElement('div');
                d.className = 'chat-message ai';
                d.textContent = content; // Set as text first
                renderMathInElement(d); // Then process for math
...
/** [NEW] Renders LaTeX math expressions within a given DOM element. */
function renderMathInElement(element) {
    ...
    for (const node of textNodes) {
        ...
        // Regex to find $...$ and $$...$$ blocks, ignoring escaped dollars
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            try {
                ...
                // If rendering fails, just add the original text back
