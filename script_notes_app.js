...
Description: Integrates the Quill.js smart editor and implements all related UX improvements.
*/
...
function saveNote() { 
    if (debounceTimer) clearTimeout(debounceTimer); 
    if (!currentNoteId || !notesCollectionRef || !quill) return; 
    const data = { 
...
function openNoteEditor(id) { 
    ...
    if (note && noteTitleInput) { 
        switchView('editor');
        currentNoteId = id; 
        noteTitleInput.value = note.title || ''; 
        quill.root.innerHTML = note.content || '';

        // UX Improvement: Clear "새 메모" on first focus
...
// ... Other functions (handleSystemReset, export/import etc.) remain the same ...
