...
Version: 13.0 (Full Enhancement Groundwork)
...
function renderNoteList() { 
    ...
    const actionBarHTML = `
        <div class="action-bar">
            <div class="action-bar-group left">
                <button id="add-new-note-btn-dynamic" class="notes-btn" title="새 메모"><svg viewBox="0 0 24 24"><path d="M19,13H13V19H11V13H5V11H11V5H13V11H19V13Z"/></svg></button>
                <button id="add-new-note-project-btn-dynamic" class="notes-btn" title="새 폴더"><svg viewBox="0 0 24 24"><path d="M4,6H2V20A2,2 0 0,0 4,22H18V20H4V6M20,2H8A2,2 0 0,0 6,4V16A2,2 0 0,0 8,18H20A2,2 0 0,0 22,16V4A2,2 0 0,0 20,2M13,13H11V10H8V8H11V5H13V8H16V10H13V13Z"/></svg></button>
            </div>
    ...
    // 5. Re-attach listeners for dynamic elements
    searchInput.addEventListener('input', renderNoteList);
    document.getElementById('add-new-note-btn-dynamic').addEventListener('click', () => addNote());
    document.getElementById('add-new-note-project-btn-dynamic').addEventListener('click', createNewNoteProject);
...
function createNoteItem(noteData) {
    ...
    item.innerHTML = `
        ...
        <div class="note-item-actions">
            <button class="item-action-btn pin-btn ${noteData.isPinned?'pinned-active':''}" title="고정"><svg viewBox="0 0 24 24" width="18" height="18"><path d="M16,12V4H17V2H7V4H8V12L6,14V16H11.2V22H12.8V16H18V14L16,12Z" /></svg></button>
            <button class="item-action-btn delete-btn" title="삭제"><svg viewBox="0 0 24 24" width="18" height="18"><path d="M6,19A2,2 0 0,0 8,21H16A2,2 0 0,0 18,19V7H6V19M8,9H16V19H8V9M15.5,4L14.5,3H9.5L8.5,4H5V6H19V4H15.5Z" /></svg></button>
        </div>
    `;
    // Add tag container placeholder
    const tagContainer = document.createElement('div');
    tagContainer.className = 'tag-container';
    // Future: Populate with tags for this note
    item.appendChild(tagContainer);
    return item;
}


// --- 3.3: Note CRUD & Other Functions (minor changes) ---
...
function saveNote() { 
    ...
    notesCollectionRef.doc(currentNoteId).update(data)
        .then(() => {
            ...
            // Future: triggerAiAutoTagging(currentNoteId);
        })
...
async function handleSystemReset() { showModal("정말로 이 캔버스의 모든 데이터를 영구적으로 삭제하시겠습니까?", async () => { if (!db || !currentUser) return; updateStatus("시스템 초기화 중...", true); const batch = db.batch(); try { [notesCollectionRef, noteProjectsCollectionRef, chatSessionsCollectionRef, projectsCollectionRef].forEach(async ref => { const snapshot = await ref.get(); snapshot.docs.forEach(doc => batch.delete(doc.ref)); }); await batch.commit(); localStorage.clear(); alert("✅ 모든 데이터가 성공적으로 삭제되었습니다. 페이지를 새로고침합니다."); location.reload(); } catch (error) { console.error("❌ 시스템 초기화 실패:", error); } }); }
function exportAllData() { /* ... unchanged ... */ }
function handleRestoreClick() { fileImporter.click(); }
function importAllData(event) { /* ... unchanged ... */ }


// --- 3.4: Stubs for Future Intelligent Features ---
function triggerAiAutoTagging(noteId) { console.log(`Future: Trigger AI auto-tagging for note ${noteId}`); }
function parseAndLinkNotes() { console.log('Future: Parse content for [[links]] and create bi-directional links.'); }
function openNoteGraph() { console.log('Future: Open a modal with a visual graph of all notes.'); }
function fetchRelatedNotes(currentNoteContent) { console.log('Future: Fetch notes related to current content.'); }
function openTemplateSelector() { console.log('Future: Open a modal to select a note template.'); }