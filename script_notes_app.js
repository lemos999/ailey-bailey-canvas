...
function openNoteEditor(id) {
    ...
    // [NEW] 글자 크기 조절을 위한 커스텀 툴바 버튼 클래스 정의
    class FontSizeButton {
        constructor(editor, options) {
            const el = document.createElement('div');
            el.className = 'toastui-editor-toolbar-group';
            ...
            button.innerHTML = `
                <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" style="vertical-align: middle;">
                    <path d="M9.62,12L12,5.67L14.37,12M11,3L5.5,17H7.5L8.62,14H15.37L16.5,17H18.5L13,3H11Z" />
                </svg>
                <span style="font-size: 14px; vertical-align: middle; margin-left: 4px;">크기</span>
                <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" style="vertical-align: middle;"><path d="M7,10L12,15L17,10H7Z" /></svg>
            `;
    
            const dropdown = document.createElement('div');
            dropdown.className = 'toastui-editor-dropdown';
            const fontSizes = options.fontSizes || [8, 10, 12, 14, 16, 18, 24, 30];
            fontSizes.forEach(size => {
                const item = document.createElement('div');
                item.className = 'toastui-editor-dropdown-item';
    ...
    // [MODIFIED] Toast UI Editor 초기화 로직 수정
    toastEditorInstance = new toastui.Editor({
        ...
        // [MODIFIED] 글자 색상 및 코드 하이라이트 플러그인 추가
        plugins: [
            toastui.Editor.plugin.codeSyntaxHighlight,
            [toastui.Editor.plugin.colorSyntax, {
                preset: ['#8C6D46', '#B99C7A', '#D4B28C', '#4B4237', '#FDF8F0', '#d9534f', '#66bb6a', '#007bff']
            }]
        ],
        // [NEW] 커스텀 툴바 아이템(글자 크기) 추가
        toolbarItems: [
            ...
            [{
                name: 'fontSize',
                el: new FontSizeButton(toastEditorInstance, { fontSizes: [12, 14, 16, 20, 24] }).el,
                tooltip: '글자 크기'
            }]
        ],
        events: {
