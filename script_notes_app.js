...
Version: 13.1 (Robust Rendering & Backup Logic)
...
// --- 3.2: Main UI Rendering (RE-ARCHITECTED for stability) ---
function setupNoteListView() {
    if (!noteListView) return;
    // This function runs only ONCE to set up the static layout.
    // It creates the non-destructive structure.
    const actionBarHTML = `
    ...
    `;
    const notesListContainerHTML = `<div id="notes-list" class="notes-list-container"></div>`; // Changed class for clarity
    noteListView.innerHTML = actionBarHTML + notesListContainerHTML;

    // Attach listeners to the now-stable Action Bar elements
...
function renderNoteList() { 
    ...
    if (!notesListContainer || !searchInput) return;
    ...
    notesListContainer.innerHTML = ''; // Only clear the list part
...
// [PATCHED] Robust backup logic with error handling
