...
function renderNoteList(renderStructure = true) { 
    ...
    // Step 1: Render the main structure only once if it doesn't exist
    if (renderStructure || !noteListView.querySelector('.action-bar')) {
        const actionBarHTML = `
            <div class="action-bar">
                <div class="action-bar-group left">
                    ...
                    <button data-action="add-project" class="notes-btn" title="새 폴더"><svg viewBox="0 0 24 24"><path d="M10,4H4C2.89,4 2,4.89 2,6V18A2,2 0 0,0 4,20H20A2,2 0 0,0 22,18V8C22,6.89 21.1,6 20,6H12L10,4Z"/></svg></button>
                </div>
    ...
    localNoteProjectsCache.forEach(project => {
        ...
        projectContainer.innerHTML = `
            <div class="note-project-header">
                ...
                <span class="note-project-title">${project.name}</span><span class="note-count">(${localNotesCache.filter(n=>n.projectId === project.id).length})</span>
            </div>
    ...
    if (fragment.children.length === 0 && searchTerm) {
        notesListContainer.innerHTML = '<div>검색 결과가 없습니다.</div>';
    } else if (fragment.children.length === 0) {
        notesListContainer.innerHTML = '<div>메모가 없습니다.</div>';
    } else {
...
function createNoteItem(noteData) {
    ...
    item.innerHTML = `<div class="note-item-content"><div class="note-item-title">${noteData.title||'무제'}</div><div class="note-item-date">${noteData.updatedAt?.toDate().toLocaleString('ko-KR',{dateStyle:'short',timeStyle:'short'})||'날짜 없음'}</div></div><div class="note-item-actions"><button class="item-action-btn pin-btn" data-action="pin-note"><svg viewBox="0 0 24 24" width="18" height="18"><path d="M16,12V4H17V2H7V4H8V12L6,14V16H11.2V22H12.8V16H18V14L16,12Z" /></svg></button><button class="item-action-btn delete-btn" data-action="delete-note"><svg viewBox="0 0 24 24" width="18" height="18"><path d="M6,19A2,2 0 0,0 8,21H16A2,2 0 0,0 18,19V7H6V19M8,9H16V19H8V9M15.5,4L14.5,3H9.5L8.5,4H5V6H19V4H15.5Z" /></svg></button></div>`;
    if (noteData.isPinned) item.querySelector('.pin-btn').classList.add('pinned-active');
    return item;
}

// --- 3.3: Note CRUD & Other Functions ---
async function addNote(content = '') { /* Unchanged */ }
...
async function handleSystemReset() {
    showModal("정말로 이 캔버스의 모든 데이터를 영구적으로 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.", async () => {
        ...
        const collectionsToDelete = [notesCollectionRef, noteProjectsCollectionRef, chatSessionsCollectionRef, projectsCollectionRef, tagsCollectionRef, noteTemplatesCollectionRef];
        const batch = db.batch();
        try {
            ...
            await batch.commit();
            localStorage.clear();
            alert("✅ 모든 데이터가 성공적으로 삭제되었습니다. 페이지를 새로고침합니다.");
            location.reload();
        } catch (error) {
