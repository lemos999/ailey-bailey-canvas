...
import { handlePopoverAskAi, handlePopoverAddNote } from '../script.js'; // Main script will expose these
...
export function handleTextSelection(e) {
    ...
    if (selectedText.length > 3) {
        ...
        // 팝오버가 화면 상단을 벗어나지 않도록 조정
        if (top < window.scrollY) {
            top = rect.bottom + window.scrollY + 10;
        }

        // 팝오버가 화면 좌우를 벗어나지 않도록 조정
...
export async function startQuiz() {
    ...
    try {
        ...
        // In a real scenario, you would make a fetch request to your AI backend here.
