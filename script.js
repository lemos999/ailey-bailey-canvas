...
--- Ailey & Bailey Canvas ---
File: script.js
Version: 7.3 (Navigation Refinement)
Architect: [Username] & System Architect Ailey
Description: Refined the Table of Contents generation logic. It now excludes overly detailed sub-topics and correctly includes the main summary sections (Final Summary, Timeline, Terminology) for a cleaner user experience.
*/
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setupNavigator() {
        ...
        // [MODIFIED] Changed selector to be more specific and cleaner
        ...
        headers.forEach((header, index) => {
            ...
            if (targetElement) {
                ...
                // Indent H3 elements for better visual hierarchy
    ...
    async function handleChatSend() {
        ...
        if(chatMessages) { chatMessages.appendChild(aiMessageDiv); chatMessages.scrollTop = chatMessages.scrollHeight; }
        const apiKey = ""; 
        ...
        try {
            ...
            if (result.candidates && result.candidates[0].content && result.candidates[0].content.parts[0]) { aiResponse = result.candidates[0].content.parts[0].text; }
            ...
            chatInput.disabled = false; chatSendBtn.disabled = false; chatInput.value = ''; chatInput.style.height = 'auto'; chatInput.focus();
        }
    }
    ...
    function renderChatHistory(mode) {
        ...
        history.forEach(msg => {
            ...
            if (content.startsWith('[PROBLEM_GENERATED]')) { msgDiv.classList.add('quiz-problem'); content = content.replace('[PROBLEM_GENERATED]', '').trim(); }
            else if (content.startsWith('[CORRECT]')) { msgDiv.classList.add('quiz-solution', 'correct'); const h = document.createElement('div'); h.className = 'solution-header correct'; h.textContent = '✅ 정답입니다!'; msgDiv.appendChild(h); content = content.replace('[CORRECT]', '').trim(); }
            else if (content.startsWith('[INCORRECT]')) { msgDiv.classList.add('quiz-solution', 'incorrect'); const h = document.createElement('div'); h.className = 'solution-header incorrect'; h.textContent = '❌ 오답입니다.'; msgDiv.appendChild(h); content = content.replace('[INCORRECT]', '').trim(); }
            const contentDiv = document.createElement('div');
            contentDiv.innerHTML = content.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br>');
            msgDiv.appendChild(contentDiv);
            if (msg.timestamp) { const t = document.createElement('div'); t.className = 'chat-timestamp'; t.textContent = new Date(msg.timestamp).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' }); msgDiv.appendChild(t); }
            if(msg.role === 'ai') { const b = document.createElement('button'); b.className = 'send-to-note-btn'; b.textContent = '메모로 보내기'; b.onclick = (e) => { addNote(`[AI 러닝메이트] ${contentDiv.textContent}`); e.target.textContent = '✅'; e.target.disabled = true; }; contentDiv.appendChild(b); }
    ...
    async function listenToChatHistory() {
        ...
        unsubscribeFromChat = chatCollectionRef.onSnapshot(doc => {
            ...
            if (doc.exists) { chatHistories = { ...defaultHistories, ...doc.data() }; } else { chatHistories = defaultHistories; }
            renderChatHistory(selectedMode);
    ...
    function setupChatModeSelector() {
        ...
        modes.forEach(mode => { const b = document.createElement('button'); b.dataset.mode = mode.id; b.innerHTML = mode.text; if (mode.id === selectedMode) b.classList.add('active'); b.addEventListener('click', () => { selectedMode = mode.id; chatQuizState = 'idle'; lastQuestion = ''; chatModeSelector.querySelectorAll('button').forEach(btn => btn.classList.remove('active')); b.classList.add('active'); if (selectedMode === 'custom') { openPromptModal(); } renderChatHistory(selectedMode); }); chatModeSelector.appendChild(b); });
    }
    ...
    function renderNoteList() {
        ...
        const filteredNotes = localNotesCache.filter(n => (n.title && n.title.toLowerCase().includes(searchTerm)) || (n.content && n.content.toLowerCase().includes(searchTerm)));
        filteredNotes.sort((a, b) => { if (a.isPinned && !b.isPinned) return -1; if (!a.isPinned && b.isPinned) return 1; return (b.updatedAt?.toMillis() || 0) - (a.updatedAt?.toMillis() || 0); });
        notesList.innerHTML = '';
        if (filteredNotes.length === 0) { notesList.innerHTML = '<div>표시할 메모가 없습니다.</div>'; return; }
        filteredNotes.forEach(note => { const item = document.createElement('div'); item.className = 'note-item'; item.dataset.id = note.id; if (note.isPinned) item.classList.add('pinned'); const date = note.updatedAt ? new Date(note.updatedAt.toMillis()).toLocaleString() : '날짜 없음'; item.innerHTML = `<div class="note-item-content"><div class="note-item-title">${note.title || '무제'}</div><div class="note-item-date">${date}</div></div><div class="note-item-actions"><button class="item-action-btn pin-btn ${note.isPinned ? 'pinned-active' : ''}" title="고정">${note.isPinned ? '📌' : '📍'}</button><button class="item-action-btn delete-btn" title="삭제">🗑️</button></div>`; notesList.appendChild(item); });
    }
    ...
    function applyFormat(format) { if (!noteContentTextarea) return; const s = noteContentTextarea.selectionStart, e = noteContentTextarea.selectionEnd; const t = noteContentTextarea.value.substring(s, e); const m = format === 'bold' ? '**' : (format === 'italic' ? '*' : '`'); noteContentTextarea.value = `${noteContentTextarea.value.substring(0, s)}${m}${t}${m}${noteContentTextarea.value.substring(e)}`; noteContentTextarea.focus(); }
    
    // Quiz
    async function startQuiz() { if (!quizModalOverlay) return; const keywords = Array.from(document.querySelectorAll('.keyword-chip')).map(k => k.textContent.trim()).join(', '); if (!keywords) { showModal("퀴즈를 생성할 키워드가 없습니다.", () => { if(quizModalOverlay) quizModalOverlay.style.display = 'none'; }); return; } if (quizContainer) quizContainer.innerHTML = '<div class="loading-indicator">퀴즈를 생성 중입니다...</div>'; if (quizResults) quizResults.innerHTML = ''; quizModalOverlay.style.display = 'flex'; try { const res = await new Promise(r => setTimeout(() => r(JSON.stringify({ "questions": [ {"question": "(e.g)정조가 젊은 인재를 양성하고 정책을 연구하기 위해 설립한 개혁의 핵심 기구는 무엇인가요?", "options": ["집현전", "장용영", "규장각", "성균관"], "answer": "규장각"}, {"question": "(e.g)정조가 상인들의 독점권을 폐지하고 자유로운 상업 활동을 보장한 경제 정책은 무엇인가요?", "options": ["과전법", "대동법", "균역법", "신해통공"], "answer": "신해통공"}, {"question": "(e.g)정조의 효심과 개혁 의지가 담겨 있으며, 정약용의 거중기 등 최신 과학 기술이 동원된 건축물은 무엇인가요?", "options": ["경복궁", "창덕궁", "수원 화성", "남한산성"], "answer": "수원 화성"} ] })), 1000)); currentQuizData = JSON.parse(res); renderQuiz(currentQuizData); } catch (e) { if (quizContainer) quizContainer.innerHTML = '퀴즈 생성에 실패했습니다. 다시 시도해주세요.'; console.error("Invalid quiz JSON", e); } }
    function renderQuiz(data) { if (!quizContainer || !data || !data.questions) return; quizContainer.innerHTML = ''; data.questions.forEach((q, i) => { const b = document.createElement('div'); b.className = 'quiz-question-block'; const p = document.createElement('p'); p.textContent = `${i + 1}. ${q.question}`; const o = document.createElement('div'); o.className = 'quiz-options'; q.options.forEach(opt => { const l = document.createElement('label'); const r = document.createElement('input'); r.type = 'radio'; r.name = `question-${i}`; r.value = opt; l.appendChild(r); l.appendChild(document.createTextNode(` ${opt}`)); o.appendChild(l); }); b.appendChild(p); b.appendChild(o); quizContainer.appendChild(b); }); }
    ...
    function initialize() {
        ...
        if (deleteHistoryBtn) deleteHistoryBtn.addEventListener('click', async () => { const m = { 'ailey_coaching': '기본 코칭', 'deep_learning': '심화 학습', 'custom': '커스텀' }; showModal(`'${m[selectedMode] || selectedMode}' 모드의 대화 기록을 정말로 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`, async () => { if(chatHistories[selectedMode]) { chatHistories[selectedMode] = []; await saveChatHistory(); } }); });
        ...
        if (quizSubmitBtn) quizSubmitBtn.addEventListener('click', () => { if (!currentQuizData || !quizResults) return; let score = 0, allAnswered = true; currentQuizData.questions.forEach((q, i) => { if (!document.querySelector(`input[name="question-${i}"]:checked`)) allAnswered = false; }); if (!allAnswered) { quizResults.textContent = "모든 문제에 답해주세요!"; quizResults.style.color = 'orange'; return; } currentQuizData.questions.forEach((q, i) => { const s = document.querySelector(`input[name="question-${i}"]:checked`); const l = document.querySelectorAll('.quiz-question-block')[i].querySelectorAll('label'); l.forEach(lbl => { const r = lbl.querySelector('input'); if (r.value === q.answer) { lbl.style.color = 'lightgreen'; lbl.style.fontWeight = 'bold'; } if (r.checked && r.value !== q.answer) lbl.style.color = 'lightcoral'; }); if (s.value === q.answer) score++; }); quizResults.textContent = `결과: ${currentQuizData.questions.length}문제 중 ${score}개를 맞혔습니다!`; quizResults.style.color = score === currentQuizData.questions.length ? 'lightgreen' : 'orange'; });
