...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderSidebarContent() {
        ...
        projectsWithFilteredSessions.forEach(project => {
            ...
            const createdAt = project.createdAt?.toDate()?.toLocaleString('ko-KR') || '정보 없음';
            const updatedAt = project.updatedAt?.toDate()?.toLocaleString('ko-KR') || createdAt;
            ...
            try {
                ...
                showModal(message, async () => {
                    try {
                        ...
                        (data.projects || []).forEach(project => { const { id, ...dataToWrite } = project; dataToWrite.createdAt = toFirestoreTimestamp(project.createdAt); batch.set(projectsCollectionRef.doc(id), dataToWrite); });
                        await batch.commit();
    ...
    function initialize() {
        ...
        if (sessionListContainer) {
            sessionListContainer.addEventListener('click', (e) => {
                ...
                } else if (projectHeader && !e.target.closest('.project-actions-btn')) {
            ...
            sessionListContainer.addEventListener('drop', async (e) => {
                ...
                if (targetProjectHeader && draggedItem) {
                    ...
                    if (sessionId && projectId) {
                        try {
                            await chatSessionsCollectionRef.doc(sessionId).update({ projectId: projectId, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
                        } catch (error) {
