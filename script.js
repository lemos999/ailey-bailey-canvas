...
--- Ailey & Bailey Canvas ---
File: script.js
Version: 9.3 (API Call Fix & Refinement)
Architect: [Username] & System Architect Ailey
Description: Fixed a critical bug where the fetch URL for the AI model was missing. Implemented the full sidebar UI logic including sections, relative time, and archiving.
*/
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // [CRITICAL] 여기에 감사관님의 Google AI API 키를 입력하세요.
    ...
    function listenToProjects() {
        return new Promise((resolve) => {
            ...
            unsubscribeFromProjects = projectsCollectionRef.orderBy("updatedAt", "desc").onSnapshot(snapshot => {
                ...
                if (newlyCreatedProjectId) {
                    const newProjectElement = document.querySelector(`.project-item[data-project-id="${newlyCreatedProjectId}"]`);
    ...
    async function deleteProject(projectId) {
        ...
        const message = `프로젝트 '${project.name}'를 삭제하시겠습니까? 프로젝트 안의 모든 대화는 '일반 대화'로 이동되며, 이 작업은 되돌릴 수 없습니다.`;
    ...
    function showProjectContextMenu(projectId, buttonElement) {
        ...
        const archiveAction = project.isArchived ? 'unarchive' : 'archive';
        const archiveText = project.isArchived ? '보관 취소' : '아카이브';
        const archiveIcon = project.isArchived 
            ? `<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M20.54 5.23L19.12 6.65C19.68 7.29 20 8.11 20 9H22C22 7.62 21.41 6.36 20.54 5.23M19.79 3.4L18.37 4.82C17.07 3.64 15.35 3 13.5 3C9.09 3 5.5 6.24 5.5 10.5V11.5H7.5V10.5C7.5 7.32 10.2 4.5 13.5 4.5C14.89 4.5 16.18 5.06 17.21 6L16.2 7H22V1L20.21 2.79L19.79 3.4M4 8H2V21A1,1 0 0,0 3,22H18V20H4V8Z" /></svg>`
            : `<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M3,3H21V7H3V3M4,8H20V21H4V8M9.5,11A0.5,0.5 0 0,0 9,11.5V13H15V11.5A0.5,0.5 0 0,0 14.5,11H9.5Z" /></svg>`;

        menu.innerHTML = `
            <button data-action="rename">${`<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18,2.9 17.35,2.9 16.96,3.29L15.12,5.12L18.87,8.87M3,17.25V21H6.75L17.81,9.93L14.06,6.18L3,17.25Z" /></svg>`}<span>이름 변경</span></button>
            <button data-action="${archiveAction}">${archiveIcon}<span>${archiveText}</span></button>
            <button data-action="delete">${`<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M6,19A2,2 0 0,0 8,21H16A2,2 0 0,0 18,19V7H6V19M8,9H16V19H8V9M15.5,4L14.5,3H9.5L8.5,4H5V6H19V4H15.5Z" /></svg>`}<span>삭제</span></button>
        `;
        
        buttonElement.closest('.project-actions').appendChild(menu);
        ...
        menu.addEventListener('click', (e) => {
            ...
            else if (action === 'unarchive') unarchiveProject(projectId);
    ...
    function startProjectRename(projectId) {
        ...
        const titleSpan = projectItem.querySelector('.project-title');
        if (!titleSpan) return;
    ...
    function groupSessionsByDate(sessions) {
        ...
        sessions.forEach(session => {
            ...
            if (sessionDate >= today) groups['오늘'].push(session);
            else if (sessionDate >= yesterday) groups['어제'].push(session);
            else if (sessionDate >= sevenDaysAgo) groups['지난 7일'].push(session);
            else groups['이전'].push(session);
        });
        
        return groups;
    }
    
    // --- [REWRITTEN] Sidebar Rendering Engine ---
    function renderSidebarContent() {
        ...
        const favoriteSessions = filteredSessions.filter(s => s.isPinned);

        const visibleProjects = localProjectsCache.filter(p => p.isArchived === isArchiveViewActive);
        const projectsWithFilteredSessions = visibleProjects.map(p => {
        ...
        const unassignedSessions = filteredSessions.filter(s => !s.projectId && !s.isPinned);

        sessionListContainer.innerHTML = '';

        // 2. Render Favorites Section
        if (favoriteSessions.length > 0) {
            sessionListContainer.appendChild(createSection('favorites', '⭐ 즐겨찾기', favoriteSessions.length, favoriteSessions.map(createSessionItem)));
        }

        // 3. Render Projects Section
        if (projectsWithFilteredSessions.length > 0) {
            sessionListContainer.appendChild(createSection('projects', isArchiveViewActive ? '🗄️ 아카이브' : '🚀 프로젝트', projectsWithFilteredSessions.length, projectsWithFilteredSessions.map(createProjectItem)));
        }

        // 4. Render Unassigned Chats Section
        if (unassignedSessions.length > 0) {
            const groupedChats = groupSessionsByDate(unassignedSessions);
            const chatElements = Object.entries(groupedChats).flatMap(([groupName, sessions]) => {
                if (sessions.length === 0) return [];
                const header = document.createElement('div');
                header.className = 'session-group-header';
                header.textContent = groupName;
                return [header, ...sessions.map(createSessionItem)];
            });
            sessionListContainer.appendChild(createSection('chats', '💬 일반 대화', unassignedSessions.length, chatElements));
        }
    }
    
    function createSection(id, title, count, children) {
        const section = document.createElement('div');
        section.className = `sidebar-section ${activeSectionStates[id] ? '' : 'collapsed'}`;
    ...
    function createProjectItem(project) {
        ...
        projectItem.className = `project-item ${project.isExpanded ? 'expanded' : ''}`;
            ...
            <div class="project-header">
                <div class="project-info">
                    ...
                    <div class="project-title-wrapper">
                        ...
                        <div class="project-meta">수정: ${formatRelativeTime(project.updatedAt)}</div>
                    </div>
                </div>
                <div class="project-actions">
                    <button class="project-action-btn add-session-to-project-btn" title="새 대화 추가">
                         <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M19,13H13V19H11V13H5V11H11V5H13V11H19V13Z" /></svg>
                    </button>
                    <button class="project-action-btn" title="프로젝트 메뉴">
        ...
        projectItem.appendChild(sessionsContainer);
    ...
    async function handleNewChat(projectId = null) { currentSessionId = null; renderSidebarContent(); if (chatMessages) { chatMessages.innerHTML = ''; chatMessages.style.display = 'none'; } if (chatWelcomeMessage) chatWelcomeMessage.style.display = 'flex'; if (chatSessionTitle) chatSessionTitle.textContent = 'AI 러닝메이트'; if (deleteSessionBtn) deleteSessionBtn.style.display = 'none'; if (chatInput) { chatInput.disabled = false; chatInput.value = ''; } if (chatSendBtn) chatSendBtn.disabled = false; if (projectId) { const query = "새로운 대화를 시작합니다."; const userMessage = { role: 'user', content: query, timestamp: new Date() }; try { const newSession = { title: "새 대화", messages: [userMessage], mode: selectedMode, projectId: projectId, isPinned: false, createdAt: firebase.firestore.FieldValue.serverTimestamp(), updatedAt: firebase.firestore.FieldValue.serverTimestamp() }; const sessionRef = await chatSessionsCollectionRef.add(newSession); selectSession(sessionRef.id); } catch (e) { console.error("Error creating new chat in project:", e); } } }
    ...
    async function toggleChatPin(sessionId) { if (!chatSessionsCollectionRef || !sessionId) return; const sessionRef = chatSessionsCollectionRef.doc(sessionId); const currentSession = localChatSessionsCache.find(s => s.id === sessionId); if (!currentSession) return; try { await sessionRef.update({ isPinned: !(currentSession.isPinned || false) }); } catch (error) { console.error("Error toggling pin status:", error); } }
    async function handleChatSend() { if (!chatInput || chatInput.disabled) return; const query = chatInput.value.trim(); if (!query) return; chatInput.disabled = true; chatSendBtn.disabled = true; const userMessage = { role: 'user', content: query, timestamp: new Date() }; let sessionRef; let messages = []; let newSessionProjectId = null; try { if (!currentSessionId) { const newSession = { title: query.substring(0, 40) + (query.length > 40 ? '...' : ''), messages: [userMessage], mode: selectedMode, projectId: null, isPinned: false, createdAt: firebase.firestore.FieldValue.serverTimestamp(), updatedAt: firebase.firestore.FieldValue.serverTimestamp() }; sessionRef = await chatSessionsCollectionRef.add(newSession); currentSessionId = sessionRef.id; messages = newSession.messages; } else { sessionRef = chatSessionsCollectionRef.doc(currentSessionId); const currentSessionData = localChatSessionsCache.find(s => s.id === currentSessionId); messages = [...(currentSessionData.messages || []), userMessage]; await sessionRef.update({ messages: messages, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }); } renderChatMessages(messages); const loadingDiv = document.createElement('div'); loadingDiv.className = 'chat-message ai'; loadingDiv.innerHTML = '<div class="loading-indicator">AI가 답변을 생성하고 있습니다...</div>'; if (chatMessages) { chatMessages.appendChild(loadingDiv); chatMessages.scrollTop = chatMessages.scrollHeight; } const apiMessages = messages.map(msg => ({ role: msg.role === 'ai' ? 'model' : 'user', parts: [{ text: msg.content }] })); 
    
    // [FIXED] The fetch URL is now correctly pointing to the API endpoint.
    ...
    async function importAllData(event) {
        ...
        reader.onload = function(e) {
            try {
                ...
                if (!data.backupVersion || data.backupVersion < '2.1') { throw new Error("호환되지 않거나 오래된 백업 파일 버전입니다."); }
    ...
    function initialize() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (newItemMenu && !newItemMenu.contains(e.target) && newItemBtn && !newItemBtn.contains(e.target)) {
        ...
        if (newItemBtn) newItemBtn.addEventListener('click', () => { newItemMenu.style.display = newItemMenu.style.display === 'block' ? 'none' : 'block'; });
        if (newItemMenu) newItemMenu.addEventListener('click', e => {
        ...
        if (toggleArchiveViewBtn) toggleArchiveViewBtn.addEventListener('click', () => {
            isArchiveViewActive = !isArchiveViewActive;
            const btnText = toggleArchiveViewBtn.querySelector('span');
            btnText.textContent = isArchiveViewActive ? '돌아가기' : '아카이브 보기';
            renderSidebarContent();
        });
        ...
        if (sessionListContainer) {
            sessionListContainer.addEventListener('click', (e) => {
                ...
                const projectActionsButton = e.target.closest('.project-actions .project-action-btn[title="프로젝트 메뉴"]');
                const addSessionButton = e.target.closest('.add-session-to-project-btn');
                const sectionHeader = e.target.closest('.sidebar-section-header');
                
                if (projectActionsButton) {
                    e.stopPropagation();
                    const projectId = projectActionsButton.closest('.project-item').dataset.projectId;
                    showProjectContextMenu(projectId, projectActionsButton);
                    return;
                }

                removeContextMenu();
                
                if (pinButton) { e.stopPropagation(); toggleChatPin(pinButton.closest('.session-item').dataset.sessionId); }
                else if (addSessionButton) { e.stopPropagation(); handleNewChat(addSessionButton.closest('.project-item').dataset.projectId); }
                else if (sessionItem) { selectSession(sessionItem.dataset.sessionId); }
                else if (projectHeader && !e.target.closest('.project-actions')) { toggleProjectExpansion(projectItem.dataset.projectId); }
                else if (sectionHeader) {
                    const sectionId = sectionHeader.closest('.sidebar-section').dataset.sectionId;
                    activeSectionStates[sectionId] = !activeSectionStates[sectionId];
            ...
            sessionListContainer.addEventListener('dragover', (e) => {
                ...
                document.querySelectorAll('.project-item.drag-over').forEach(el => el.classList.remove('drag-over'));
                if (targetProject) targetProject.classList.add('drag-over');
            });
            sessionListContainer.addEventListener('drop', async (e) => {
                e.preventDefault();
                document.querySelectorAll('.project-item.drag-over').forEach(el => el.classList.remove('drag-over'));
                const targetProjectItem = e.target.closest('.project-item');
                if (targetProjectItem && draggedItem) {
                    const sessionId = e.dataTransfer.getData('text/plain');
                    const projectId = targetProjectItem.dataset.projectId;
                    try {
