...
document.addEventListener('DOMContentLoaded', function () {
    ...
    i.dataset.noteId = note.id; // Use noteId for clarity
    i.draggable = true;
    if (note.isPinned) i.classList.add('pinned');
...
function renderNoteList() {
    if (!notesList || !searchInput || !localProjectsCache) return;
    const searchTerm = searchInput.value.toLowerCase();
    notesList.innerHTML = ''; 

    const filteredNotes = localNotesCache.filter(n => 
        (n.title || '무제').toLowerCase().includes(searchTerm) || 
        (n.content || '').toLowerCase().includes(searchTerm)
    );

    const fragment = document.createDocumentFragment();

    const projectsWithNotes = localProjectsCache.map(p => {
        return {
            ...p,
            notes: filteredNotes
        ...
        return p.name.toLowerCase().includes(searchTerm) || p.notes.length > 0;
    }).sort((a, b) => (a.createdAt?.toMillis() || 0) - (b.createdAt?.toMillis() || 0));

    if (projectsWithNotes.length > 0) {
        const projectGroupHeader = document.createElement('div');
        projectGroupHeader.className = 'session-group-header';
        projectGroupHeader.textContent = '📁 프로젝트 메모';
        fragment.appendChild(projectGroupHeader);

        projectsWithNotes.forEach(project => {
            const projectContainer = document.createElement('div');
            projectContainer.className = 'project-container note-project-container';
            projectContainer.dataset.projectId = project.id;
    
            const projectHeader = document.createElement('div');
            projectHeader.className = 'project-header note-drop-target';
            projectHeader.innerHTML = `
                <span class="project-toggle-icon ${project.isExpanded ? 'expanded' : ''}">
                    <svg viewBox="0 0 24 24" width="16" height="16"><path d="M8.59,16.58L13.17,12L8.59,7.41L10,6L16,12L10,18L8.59,16.58Z" /></svg>
                </span>
            ...
            project.notes.forEach(note => {
                notesContainer.appendChild(createNoteItem(note));
            });
    ...
    if (unassignedNotes.length > 0) {
        const generalGroupHeader = document.createElement('div');
        generalGroupHeader.className = 'session-group-header note-drop-target general-notes-header';
        generalGroupHeader.dataset.projectId = 'null';
        generalGroupHeader.textContent = '📝 일반 메모';
        fragment.appendChild(generalGroupHeader);
    ...
    async function addNote(content = '') {
        ...
        // By default, new notes are not assigned to any project.
    ...
    function initialize() {
        ...
        if (notesList) {
            notesList.addEventListener('click', e => {
                ...
                // Project header toggling
                const projectHeader = e.target.closest('.project-header.note-drop-target');
                if (projectHeader) {
                    const projectId = projectHeader.closest('.project-container').dataset.projectId;
                    const project = localProjectsCache.find(p => p.id === projectId);
                    if (project) {
                        project.isExpanded = !project.isExpanded;
                        renderNoteList(); // Re-render to reflect the change
                    }
                }
            });
            ...
            notesList.addEventListener('dragend', () => {
                ...
                document.querySelectorAll('.note-drop-target.drag-over').forEach(el => el.classList.remove('drag-over'));
            });

            notesList.addEventListener('dragover', (e) => {
                e.preventDefault();
                const target = e.target.closest('.note-drop-target');
                document.querySelectorAll('.note-drop-target.drag-over').forEach(el => el.classList.remove('drag-over'));
                
                if (draggedNote && target) {
                    const sourceNoteId = draggedNote.dataset.noteId;
                    const sourceNote = localNotesCache.find(n => n.id === sourceNoteId);
                    const targetProjectId = target.dataset.projectId;
                    
                    const sourceProjectId = sourceNote.projectId || 'null';
            ...
            notesList.addEventListener('drop', async (e) => {
                ...
                let targetProjectId = target.dataset.projectId;
