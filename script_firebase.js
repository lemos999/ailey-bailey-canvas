...
// Listener for Note Projects
export function listenToNoteProjects() {
...
export function listenToTags() {
    return new Promise((resolve) => {
        ...
        unsubscribeFromTags = tagsCollectionRef.onSnapshot(snapshot => {
            ...
            // Potentially re-render parts of the UI if tags are displayed globally
            resolve();
        }, error => { console.error("Tags listener error:", error); resolve(); });
    });
}

// NEW: Listener for Note Templates
export function listenToNoteTemplates() {
    return new Promise((resolve) => {
