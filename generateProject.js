// ===================================================================
//      Ailey & Bailey Canvas: 프로젝트 구조 자동 생성기 (v1.0)
//      - Author: System Architect
//      - Description: 분산된 소스 파일을 체계적인 폴더 구조로 재구성하고
//        최종 배포용 파일을 자동으로 생성합니다.
// ===================================================================
const fs = require('fs');
const path = require('path');

console.log('>>> 프로젝트 구조 생성을 시작합니다...');

// --- 1. 프로젝트 설계도 (Source Files Definition) ---
// 이 객체는 모든 소스 파일의 경로와 내용을 정의하는 '단일 진실 공급원'입니다.
// 여기에 정의된 내용을 기반으로 모든 파일과 폴더가 생성됩니다.
// [RULE] 파일 내부의 코드는 절대 수정하지 않습니다. (No-Refactoring)

const sourceFiles = {
    // --- CSS Files ---
    // 역할에 따라 css/components 와 css/layout 폴더로 재배치합니다.

    'src/css/layout/style_base.css': `
/*
--- Ailey & Bailey Canvas ---
File: style_base.css
...
`,
    ...
    'src/css/layout/style_navigation.css': `
/*
--- Ailey & Bailey Canvas ---
...
`,
    ...
    'src/css/components/style_widgets.css': `
/*
--- Ailey & Bailey Canvas ---
...
`,
    ...
    'src/css/components/style_panels.css': `
/*
--- Ailey & Bailey Canvas ---
File: style_panels.css
Version: 11.0 (Modular CSS Structure)
Architect: [Username] & System Architect Ailey
Description: Defines the common styles for all draggable panels used in the application, such as the Chat and Notes panels. This includes the main panel container, header, and close buttons.
*/
...
`,
    ...
    'src/css/components/style_chat.css': `
/*
--- Ailey & Bailey Canvas ---
File: style_chat.css
Version: 11.0 (Modular CSS Structure)
...
`,
    ...
    'src/css/components/style_notes.css': `
/*
--- Ailey & Bailey Canvas ---
File: style_notes.css
Version: 11.0 (Modular CSS Structure)
Architect: [Username] & System Architect Ailey
Description: Contains all styles related to the Notes App panel, including the note list view, search and action buttons, individual note items, and the note editor view with its toolbar.
*/

/* Notes App Panel Styles */
...
#note-list-view .panel-header .header-button-group { display: flex; gap: 10px; flex-wrap: wrap; }
#note-list-view .list-header { padding: 10px 15px; display: flex; gap: 10px; align-items: center; border-bottom: 1px solid var(--border-color-dark); }
body:not(.dark-mode) #note-list-view .list-header { border-bottom-color: var(--border-color-light); }
#search-input { flex-grow: 1; padding: 8px 12px; border-radius: 20px; border: 1px solid var(--border-color-dark); background-color: var(--quote-bg-dark); color: var(--text-color-dark); }
body:not(.dark-mode) #search-input { border-color: var(--border-color-light); background-color: var(--quote-bg-light); color: var(--text-color-light); }
.notes-btn { padding: 8px 15px; border-radius: 20px; border: none; cursor: pointer; background-color: var(--h2-color-dark); color: white; display: flex; align-items: center; justify-content: center; gap: 6px; }
...
body.dark-mode .note-item:hover { background-color: var(--quote-bg-dark); }
.note-item.pinned { border-left: 4px solid var(--pin-color); }
body:not(.dark-mode) .note-item.pinned { background-color: #fff9e8; }
body.dark-mode .note-item.pinned { background-color: #2e281d; }
...
.item-action-btn { background: none; border: none; cursor: pointer; font-size: 1.2em; padding: 5px; border-radius: 50%; line-height: 0; }
.item-action-btn:hover { background-color: rgba(128, 128, 128, 0.2); }
.item-action-btn svg { fill: currentColor; }
...
#note-title-input { font-size: 1.2em; font-weight: bold; width: 100%; border: none; background: transparent; padding: 10px 15px; color: var(--text-color-dark); }
body:not(.dark-mode) #note-title-input { color: var(--text-color-light); }
.format-toolbar { display: flex; gap: 10px; padding: 8px 15px; border-bottom: 1px solid var(--border-color-dark); }
body:not(.dark-mode) .format-toolbar { border-bottom-color: var(--border-color-light); }
.format-btn { background: none; border: 1px solid; border-radius: 5px; padding: 5px 10px; cursor: pointer; }
#note-content-textarea { flex-grow: 1; width: 100%; border: none; background: transparent; padding: 15px; resize: none; font-family: 'Gowun Batang', serif; font-size: 1em; color: var(--text-color-dark); box-sizing: border-box; }
body:not(.dark-mode) #note-content-textarea { color: var(--text-color-light); }
#auto-save-status { font-size: 0.8em; padding: 5px 15px; text-align: right; opacity: 0.7; }
`,

    // --- CSS Entry Point ---
    // 모든 CSS 모듈을 @import 하여 하나로 통합하는 역할을 합니다.
    'src/css/main.css': `
/*
--- Ailey & Bailey Canvas ---
File: src/css/main.css (CSS Entry Point)
Version: 12.0 (Restructured)
Description: Imports all modularized CSS files in the correct dependency order.
             This file will be compiled into 'dist/style.css'.
*/

/* 1. Base styles, variables, and global element styling */
@import url('./layout/style_base.css');

/* 2. Core layout and main navigation components */
@import url('./layout/style_navigation.css');

/* 3. Reusable widgets and overlays (tooltips, modals) */
@import url('./components/style_widgets.css');

/* 4. Draggable panel base styles (common for chat, notes) */
@import url('./components/style_panels.css');

/* 5. Specific, complex component styles */
@import url('./components/style_chat.css');
@import url('./components/style_notes.css');
`
};

// ... (다음 파트에서 JS 파일 및 실행 로직이 추가됩니다) ...