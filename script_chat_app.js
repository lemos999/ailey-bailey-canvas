...
function handleNewChat() { 
    ...
    // Key changes for UX: Make the chat area visible immediately
    if (chatMessages) {
        // Clear only dynamically added message elements, preserving the welcome message element
        chatMessages.querySelectorAll('.chat-message, .ai-response-container, .reasoning-block').forEach(el => el.remove());
        chatMessages.style.display = 'flex'; // Make the container take up space
    }

    if (chatWelcomeMessage) {
        chatWelcomeMessage.style.display = 'flex'; // Ensure welcome message is visible
        const p = chatWelcomeMessage.querySelector('p');
        if (p) p.textContent = "아래 입력창에 질문을 입력하여 대화를 시작해보세요!"; // Update text
    }
...
function renderChatMessages(sessionData) {
    ...
    // If a session is selected (even an empty one), hide the initial welcome message
