/* Auto-generated bundle from 2025-08-08T10:25:53.963Z */
...
function initializeTooltips(container) { 
    (container || document).querySelectorAll('.keyword-chip[data-tooltip]').forEach(chip => { 
        if (chip.querySelector('.tooltip')) { 
            chip.classList.add('has-tooltip'); 
            chip.querySelector('.tooltip').textContent = chip.dataset.tooltip; 
        } 
    }); 
    (container || document).querySelectorAll('.content-section strong[data-tooltip]').forEach(highlight => { 
...
function setupNavigator(learningContent) { 
...
Version: 2.0 (Dynamic Injection Architecture)
Description: The main entry point for the application. Initializes the app,
             attaches all primary event listeners, and now also triggers the
             initial dynamic content loading from the AI.
*/
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initialize() {
        ...
        initializeFirebase().then(() => { 
            ...
            // setupNavigator(); // This will be called after content injection
            setupChatModeSelector(); 
            // initializeTooltips(); // This will be called after content injection
    ...
    /**
     * [NEW] AI에게 초기 학습 콘텐츠를 요청하고 페이지에 주입합니다.
     */
    async function loadInitialContent() {
        if (!dynamicContentArea) return;
        dynamicContentArea.innerHTML = '<div class="loading-indicator">AI가 학습 콘텐츠를 생성하는 중입니다...</div>';

        try {
            // This is a placeholder for the actual call to the AI.
            // In a real scenario, this would involve sending a request to a backend or directly to the AI API.
            // For this simulation, we will use a predefined HTML fragment.
            const topic = "정조대왕의 역사"; // Example topic
            const aiHtmlFragment = await getAiContentFragment(topic);

            dynamicContentArea.innerHTML = aiHtmlFragment;

            // [CRITICAL] Hydrate the new content
            const learningContent = document.getElementById('learning-content');
            if (learningContent) {
                setupNavigator(learningContent);
                initializeTooltips(learningContent);
            } else {
               console.error("Failed to find #learning-content after injection.");
            }

        } catch (error) {
            console.error("Error loading initial content:", error);
            dynamicContentArea.innerHTML = '<div class="error-message">콘텐츠를 불러오는 데 실패했습니다.</div>';
        }
    }

    /**
     * [NEW] AI 콘텐츠 생성을 시뮬레이션하는 함수입니다.
     * 실제 환경에서는 이 부분이 Gemini API와 통신하게 됩니다.
     * @param {string} topic - The topic for the AI to generate content about.
     * @returns {Promise<string>} A promise that resolves to an HTML fragment.
     */
    function getAiContentFragment(topic) {
        console.log(`AI에게 "${topic}"에 대한 콘텐츠를 요청합니다...`);
            ...
            setTimeout(() => {
                ...
                <div class="wrapper">
                    ...
                    <div class="container" id="learning-content">
                        <div class="header">
                            <h1>(시뮬레이션) ${topic}</h1>
                            <p class="subtitle">AI가 동적으로 생성한 학습 콘텐츠입니다.</p>
                        </div>
                        <div id="section-1" class="content-section">
                            <h2>🎯 오늘의 학습 목표</h2>
                            <blockquote>정조의 개혁 정치와 문화적 업적을 이해하고, 그의 리더십이 현대에 주는 시사점을 파악합니다.</blockquote>
                        </div>
                        <div id="section-2" class="content-section">
                            <h2>🔑 핵심 키워드</h2>
                            <div class="keyword-list">
                                <span class="keyword-chip" data-tooltip="정조가 왕권 강화를 위해 창덕궁에 설치한 학술 기관 겸 왕실 도서관입니다.">규장각<span class="tooltip"></span></span>
                                <span class="keyword-chip" data-tooltip="정조가 아버지 사도세자의 묘를 옮기면서 축조한 신도시이자 군사 기지입니다.">수원 화성<span class="tooltip"></span></span>
                            </div>
                        </div>
                        <div id="section-3" class="content-section">
                            <h2>📖 핵심 원리와 개념</h2>
                            <p>이곳의 내용은 <strong>bundle/main.js</strong>의 초기화 로직에 의해 동적으로 주입되었습니다. AI는 이제 전체 HTML이 아닌, 이 콘텐츠 조각(Fragment)만을 생성하여 전달하는 역할을 맡습니다.</p>
                        </div>
                    </div>
                </div>`;
    ...
    function attachEventListeners() {
        ...
        document.addEventListener('click', (e) => { 
            // Text selection logic needs to account for dynamic content
            const learningContent = document.getElementById('learning-content');
            if (learningContent && learningContent.contains(e.target)) {
        ...
        // TOC Toggle listener now needs to check for the dynamic nav
        if (tocToggleBtn) tocToggleBtn.addEventListener('click', () => { 
             const mainWrapper = document.querySelector('#dynamic-content-area .wrapper');
             if(mainWrapper) mainWrapper.classList.toggle('toc-hidden');
