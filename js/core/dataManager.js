...
Description: Handles data operations like backup, restore, and system reset.
*/
import { showModal } from './ui.js';
import { getState } from './state.js';

export async function handleSystemReset(db, auth) {
    const message = "정말로 이 캔버스의 모든 프로젝트, 채팅, 메모 기록을 영구적으로 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.";
    showModal(message, async () => {
        if (!db || !auth.currentUser) {
            alert("초기화 실패: DB 연결 또는 인증 상태를 확인해주세요.");
            return;
        }
        ...
        try {
            ...
            const collections = [state.notesCollection, state.chatSessionsCollection, state.projectsCollection];
...
export function exportAllData() {
    const state = getState();
    const { notesCache, chatSessionsCache, projectsCache } = state;

    if (notesCache.length === 0 && chatSessionsCache.length === 0 && projectsCache.length === 0) {
    ...
    const dataToExport = {
        ...
        notes: notesCache.map(processTimestamp),
        chatSessions: chatSessionsCache.map(processTimestamp),
        projects: projectsCache.map(processTimestamp)
    };
...
export function handleRestoreClick() {
    const { fileImporter } = getState().elements;
    if (fileImporter) fileImporter.click();
}

export function importAllData(event, db) {
        ...
        try {
            ...
            showModal(message, async () => {
                try {
                    ...
                    const state = getState();
                    const { notesCollection, chatSessionsCollection, projectsCollection } = state;
                    ...
                    (data.chatSessions || []).forEach(session => {
                        ...
                        batch.set(chatSessionsCollection.doc(id), dataToWrite);
                    });
                    (data.projects || []).forEach(project => {
                        ...
                        batch.set(projectsCollection.doc(id), dataToWrite);
                    });
