...
Description: Manages all aspects of the user's personal API key (BYOK) settings, including modal UI, key verification, model fetching, and saving preferences to local storage.
*/
...
let apiSettingsModalOverlay, apiKeyInput, verifyApiKeyBtn, apiKeyStatus,
    ...
    apiSettingsSaveBtn, apiSettingsCancelBtn, aiModelSelector;
...
export function initializeApiManager() {
    ...
    aiModelSelector = document.getElementById('ai-model-selector'); // For header selector
    ...
    if (aiModelSelector) {
        aiModelSelector.addEventListener('change', handleModelSelectionChange);
    }
...
function loadApiSettings() {
    ...
    if (savedSettings) {
        let loadedSettings = JSON.parse(savedSettings);
        if (!loadedSettings.tokenUsage) { loadedSettings.tokenUsage = { prompt: 0, completion: 0 }; }
        if (!loadedSettings.availableModels) { loadedSettings.availableModels = []; }
        state.setUserApiSettings(loadedSettings);
    }
}
