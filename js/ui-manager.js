...
Description: Manages generic UI interactions and components that are not specific
to a single feature, such as the clock, tooltips, draggable panels, and modals.
*/

import * as state from './state.js';

let systemInfoWidget, selectionPopover, quizModalOverlay, customModal, promptModalOverlay;

export function initializeUIManager() {
    // Query for elements this manager is responsible for
    ...
    // Setup global event listeners for UI
    document.addEventListener('click', (e) => {
        handleTextSelection(e);
        const contextMenu = document.querySelector('.session-context-menu, .project-context-menu');
        if (contextMenu && !e.target.closest('.session-context-menu, .project-context-menu')) {
            removeContextMenu();
        }
    });
...
function setupSystemInfoWidget() {
    ...
    tooltip.innerHTML = `<div><strong>Canvas ID:</strong> ${state.canvasId}</div><div><strong>User ID:</strong> ${state.currentUser.uid}</div>`;
...
function handleTextSelection(e) {
    ...
    if (state.currentOpenContextMenu) removeContextMenu();
...
export function showModal(message, onConfirm) {
    ...
    if (!modalMessage || !modalConfirmBtn || !modalCancelBtn) return;
