...
Description: Manages common UI interactions across the application, such as panel dragging, modals, tooltips, navigation, and other generic interface elements.
*/

import * as state from './state.js';
import { handlePopoverAskAi, handlePopoverAddNote } from './chat-core.js'; 
import { addNote } from './notes-manager.js';

let selectionPopover, customModal, modalMessage, modalConfirmBtn, modalCancelBtn;
let quizModalOverlay, quizContainer, quizResults, quizSubmitBtn;
...
export function setupSystemInfoWidget() {
    ...
    if (!systemInfoWidget || !state.currentUser) return;
    ...
    tooltip.innerHTML = `<div><strong>Canvas ID:</strong> ${canvasId}</div><div><strong>User ID:</strong> ${state.currentUser.uid}</div>`;
...
export function handleTextSelection(e) {
    ...
    if (selectedText.length > 3) {
        ...
        selectionPopover.style.top = `${top < window.scrollY ? rect.bottom + window.scrollY + 10 : top}px`;
        selectionPopover.style.left = `${Math.max(5, Math.min(left, window.innerWidth - selectionPopover.offsetWidth - 5))}px`;
...
export function setupPopover() {
    ...
    if (popoverAskAi) {
        popoverAskAi.addEventListener('click', () => {
            handlePopoverAskAi(state.lastSelectedText);
            selectionPopover.style.display = 'none';
        });
    }
    if (popoverAddNote) {
        popoverAddNote.addEventListener('click', () => {
            handlePopoverAddNote(state.lastSelectedText);
            selectionPopover.style.display = 'none';
        });
    }
}
...
export function setupQuiz() {
    ...
    if (quizSubmitBtn) {
        quizSubmitBtn.addEventListener('click', () => {
            ...
            quizResults.textContent = `결과: ${state.currentQuizData.questions.length} 중 ${score} 정답!`;
        });
    }
...
async function startQuiz() {
    ...
    try {
        ...
        state.setCurrentQuizData(JSON.parse(res));
